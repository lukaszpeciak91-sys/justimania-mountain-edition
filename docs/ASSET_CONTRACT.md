...
- `menu-justyna-kaya.png` is a user-owned transparent PNG rendered directly as a separate, non-interactive menu foreground layer; Codex must never create, crop, resize, optimize, convert, re-encode, overwrite, or supply a placeholder for it. If it is absent, the complete menu and its controls remain available.
- The menu duo preserves its source aspect ratio with one uniform runtime scale, targeting 330 logical px tall with edge-fit clamping when required. It automatically fades in and moves upward 22 px over 420 ms with `Quad.easeOut`; reduced motion completes in 1 ms. This decorative entrance never gates the immediately available PLAY and RETURN controls. Its depth is background (`0`) < duo (`10`) < title/subtitle (`20`) < controls. Final placement, transparency, silhouette visibility, and title/control clearance require validation with the supplied binary on a physical portrait phone.
