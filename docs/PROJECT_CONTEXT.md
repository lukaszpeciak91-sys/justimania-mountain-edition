...
- That same first tap gently introduces the optional Justyna + Kaya transparent PNG as a separate decorative foreground: a 22 px upward fade over 420 ms (`1 ms` for reduced motion), uniformly scaled toward 330 logical px tall and lower-right positioned. It has no input or hit area, sits below both title lines and START, and is omitted safely when missing. The user-owned binary is never modified; final composition requires physical portrait-device validation.
