...
- `BootScene` preloads every canonical manually supplied asset, including the user-owned `public/assets/ui/menu-justyna-kaya.png` under `menu-justyna-kaya` and `public/assets/audio/game-theme.mp3` under `game-theme`, and reports development-only failures. Every consumer tolerates a missing asset without preventing boot.
- `GameScene` composes `Player`, `PlatformManager`, `BackgroundManager`, `AscentTracker`, and `CheckpointManager` with separate ownership.
- Justyna uses four state frames, runtime horizontal flipping, a stable normalized collision body, unchanged automatic bounce, and light acceleration/drag-based horizontal momentum shared by both editions.
...
- The menu presents the Bungee display face above a contrasting coral-red, italic Barlow Condensed edition lockup. The subtitle and the DOM PLAY and RETURN controls are visible and interactive immediately, without a background-tap reveal gate. Both faces are loaded from Google Fonts with explicit local fallback stacks.
- The optional Justyna + Kaya transparent PNG automatically enters with a 22 px upward fade over 420 ms (`1 ms` for reduced motion), uniformly scaled toward 330 logical px tall and lower-right positioned. Its decorative animation does not gate either menu control. It has no input or hit area, sits below both title lines and controls, and is omitted safely when missing. The user-owned binary is never modified; final composition requires physical portrait-device validation.
- PLAY and TAP TO RESTART share a compact rounded forest-green button treatment with coral outline, warm-cream Barlow Condensed labels, and the same restrained press response; their dimensions remain appropriate to their respective menu and Game Over contexts.
...
No user-owned binary generation or modification, final audio mix/balance, menu music, sound effects, mute/settings UI, enemies, power-ups, backend, monetization, analytics, or generic audio framework layer. Difficulty V4 is source/logic validated; final balance remains subject to physical Android playtesting.
