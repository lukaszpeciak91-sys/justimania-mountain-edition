...
export default class PlatformManager {
  ...
  add(x, y, width = 120) {
    ...
    if (textureAvailable(this.scene, ASSETS.platform)) {
      platform = this.scene.add.nineslice(x, y, ASSETS.platform.key, null, width, PLATFORM_HEIGHT, CAP_WIDTH, CAP_WIDTH, 7, 7);
    } else {
      platform = this.scene.add.rectangle(x, y, width, 20, 0x526f45).setStrokeStyle(3, 0x30462c);
    }
    this.scene.physics.add.existing(platform, true);
    platform.body.setSize(width, 18).setOffset(0, 0).updateFromGameObject();
